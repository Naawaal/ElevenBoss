-- 084: Evolving may coexist with InXI (match-based evolution tracks).
-- Allow start_evolution from InXI; assign_xi / bench / match_include while Evolving.
-- Aligns US-42.2 matrix with US-18 match-appearance objectives (closes dead-end: bench→start→can't play).
...
BEGIN
    ...
    IF v_in_xi AND v_busy >= 1 THEN
        -- Evolving + InXI is legal (match tracks). Other busy+XI remains conflict.
        IF NOT (
            v_evolving
            AND NOT v_retired
            AND NOT v_listed
    ...
    ELSIF p_action = 'assign_xi' THEN
        IF v_primary NOT IN ('RosterFree', 'Evolving') THEN
            RAISE EXCEPTION 'CARD_STATE: % blocks assign_xi', v_primary;
        END IF;
    ELSIF p_action = 'bench' THEN
        IF v_primary NOT IN ('InXI', 'Evolving') THEN
            RAISE EXCEPTION 'CARD_STATE: % blocks bench', v_primary;
        END IF;
        IF v_primary = 'Evolving' AND NOT v_in_xi THEN
            RAISE EXCEPTION 'CARD_STATE: Evolving blocks bench';
        END IF;
    ELSIF p_action = 'match_include' THEN
        IF v_primary NOT IN ('RosterFree', 'InXI', 'Evolving') THEN
...
BEGIN
    IF to_regprocedure('public.assert_card_action_allowed(bigint,uuid,text)') IS NULL THEN
        RAISE EXCEPTION '084 guard: assert_card_action_allowed missing';
    END IF;
END $$;
