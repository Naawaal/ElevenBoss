...
- **AC-39n (094 manager card gifts):** On migration apply, every **existing non-AI** registered manager is snapshotted for a one-time **Epic** card gift (OVR 75–84, random position, POT within Epic cap). Manager `976054227459776582` also receives a second gift: **Legendary MID @ exactly 92 OVR** (POT 92–99). Cards are prepared once (deterministic seed from campaign+owner+slot), claimed atomically (all outstanding gifts in one Claim), land as free roster (not auto-XI), and open `card_ownership_history` via `ensure_card_ownership_open`. Startup DMs send a persistent Claim button; `discord.Forbidden` records `dm_status=blocked` (no endless restart retries) and `/development` still shows **Claim Card Gift(s)**. Gated by `manager_card_gifts_enabled`; tracked in `manager_card_gifts`. US-42.1 / 42.2 / 42.9 apply (ownership, free roster state, potential integrity).
