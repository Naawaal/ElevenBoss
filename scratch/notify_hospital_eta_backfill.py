"""Best-effort Medical Update DMs for 012 early discharges.

Usage:
  python scratch/notify_hospital_eta_backfill.py --from-file path.json
  echo '[{"owner_id":1,"name":"X","tier":3}]' | python scratch/notify_hospital_eta_backfill.py
"""
...
logger = logging.getLogger("notify_hospital_eta_backfill")
...
def _load_entries(args: argparse.Namespace) -> list[dict[str, Any]]:
    ...
    if not isinstance(data, list):
        raise SystemExit("Expected a JSON array of early_discharged entries")
...
def _message_for(names: list[str]) -> str:
    ...
    return (
        ...
        "due to advancements in our medical protocols.\n"
        "Check `/profile` → **Manage Hospital** for the latest roster status."
    )
