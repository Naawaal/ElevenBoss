...
async def season_aging_job(bot: commands.Bot) -> None:
    ...
    try:
        from apps.discord_bot.core.job_claims import run_claimed_job, utc_week_window
...
async def youth_intake_job(bot: commands.Bot) -> None:
    ...
    try:
        ...
        await run_claimed_job(db, "youth_intake", utc_week_window(), _work)
    except Exception:
...
async def regen_pool_job(bot: commands.Bot) -> None:
    ...
    try:
        ...
        await run_claimed_job(db, "regen_pool", utc_week_window(), _work)
    except Exception:
...
async def daily_recovery_job(bot: commands.Bot) -> None:
    ...
    try:
        from apps.discord_bot.core.job_claims import run_claimed_job, utc_day_window
        ...
        await run_claimed_job(db, "daily_recovery", utc_day_window(), _work)
    except Exception:
...
async def academy_growth_job(bot: commands.Bot) -> None:
    ...
    try:
        ...
        await run_claimed_job(db, "academy_growth", utc_day_window(), _work)
    except Exception:
...
async def transfer_listing_expiry_job(bot: commands.Bot) -> None:
    ...
    try:
        from apps.discord_bot.core.job_claims import run_claimed_job, utc_hour_window
        ...
        await run_claimed_job(db, "transfer_listing_expiry", utc_hour_window(), _work)
    except Exception:
...
async def weekly_payroll_job(bot: commands.Bot) -> None:
    ...
    try:
        ...
        await run_claimed_job(db, "weekly_payroll", utc_week_window(), _work)
    except Exception:
    ...
    try:
        ...
        async def _work() -> None:
            ...
            for div_idx, div_name in enumerate(DIVISIONS):
                ...
                if div_idx > 0:
                    relegations.extend(res.relegated_ids)
        ...
        await run_claimed_job(db, "weekly_league_reset", utc_week_window(), _work)
...
async def league_state_machine_job(bot: commands.Bot) -> None:
    ...
    try:
        ...
        await run_claimed_job(db, "league_state_machine", utc_day_window(), _work)
    except Exception:
        logger.exception("League state machine job failed.")
...
async def league_matchday_reminder_job(bot: commands.Bot) -> None:
    ...
    try:
        ...
        async def _work() -> None:
            ...
            seasons_res = await db.table("league_seasons").select(
                "id, current_matchday, league_id"
            ).eq("status", "active").execute()
            for season in seasons_res.data or []:
                fix_res = await db.table("league_fixtures").select(
                    "window_end, home_team_id, away_team_id"
                ).eq(
                    "season_id", season["id"]
                ...
                window_end = datetime.fromisoformat(
                    fix_res.data[0]["window_end"].replace("Z", "+00:00")
                )
                ...
                for p in parts_res.data or []:
                    ...
                    existing = await db.table("league_matchday_reminders").select("player_id").eq(
                        "season_id", season["id"]
                    ).eq("matchday", season["current_matchday"]).eq(
                        "player_id", pid
                    ).maybe_single().execute()
        ...
        await run_claimed_job(db, "league_matchday_reminder", utc_hour_window(), _work)
    except Exception as e:
