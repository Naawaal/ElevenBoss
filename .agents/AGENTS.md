...
* **Active Fatigue Recovery (009 / US-39 extension; 011 QoL; 012 backfill):** Recovery Session via RPC `process_recovery_session` (+40 fatigue, 0 XP, 0 coins, energy from `fatigue_recovery_energy` default 5, shares drill caps) under `/development` Training Drills. Daily passive via `process_daily_recovery` = `25 + (training_ground_level × 5)` for non-hospital cards; bench +25; base drain 18; injury bases 1/4/7. Open pre-011 hospital/overflow clocks: one-shot idempotent RPC `backfill_injury_eta_fairness` (migration 057). Never mutate `player_cards.fatigue` outside fatigue RPCs (`apply_match_fatigue`, `process_recovery_session`, `process_daily_recovery`, hospital admit/discharge / backfill recovery paths). Pure mirrors in `packages/player_engine/fatigue.py` / `injury_math.py`.
...
* **Development hub:** drills (skill + Recovery Session), fusion, evolutions, skill allocation / mentor, claim level rewards only (`development_cog.py`).
