# packages/player_engine/player_engine/__init__.py
from __future__ import annotations

from .age_manager import (
    DEFAULT_LIFECYCLE,
    LifecyclePhase,
    age_from_dob,
    apply_xp_age_multiplier,
    can_renew_contract,
    dob_from_age,
    format_lifecycle_display,
    lifecycle_phase,
    lifecycle_phase_emoji,
    lifecycle_phase_label,
    xp_multiplier,
    yearly_stat_decline,
)
from .archetypes import ARCHETYPES, ArchetypeDef, archetypes_for, roll_archetype
from .created_card import CreatedPlayerCard
from .player_factory import balance_true_ovr, create_player_card, roll_creation_age
from .youth_intake import generate_youth_intake_cards
from .youth_math import (
    AGE_OUT_DEFAULT,
    AGE_WARN_DEFAULT,
    AGING_DECAY_MAX_DEFAULT,
    POINTS_PER_OVR,
    READY_OVR_BY_LEVEL,
    READY_OVR_DEFAULT,
    GrowthResult,
    academy_daily_points,
    apply_academy_aging_decay,
    apply_academy_tick,
    is_promotion_ready,
    ready_ovr_for_level,
    should_age_out,
    should_age_warn,
    star_band,
)
from .scout_ranges import (
    DEEP_MIN_RANGE,
    init_visible_range,
    narrow_range,
    next_assessment_level,
    star_band_from_interval,
)
from .regen_pool import generate_regen_from_retired, regen_rarity_for_ovr
from .config import GameConfig
from .generated_player import GeneratedPlayer
from .potential import (
    RARITY_POT_CAPS,
    apply_dynamic_potential_boost,
    clamp_potential,
    effective_potential,
    format_potential_display,
    generate_potential,
    potential_tier_label,
    rarity_potential_cap,
    validate_potential_integrity,
)
from .engine import (
    calculate_level,
    roll_dynamic_potential,
    calculate_contract_renewal_cost,
    calculate_true_ovr,
    apply_match_form,
)
from .progression_gates import (
    STAT_KEYS,
    can_allocate_skill_point,
    can_gain_stat_progression,
    can_start_evolution_track,
    detect_stat_inflation,
    evolution_reward_steps,
    evolution_track_stat_col,
    rebalance_stats_to_ovr,
    simulate_legacy_stat_drill,
    stats_from_card,
)
from .progression import (
    L_MAX,
    POINTS_PER_LEVEL,
    FUSION_DAILY_LIMIT,
    RETRO_SCALE_PCT,
    RETRO_MAX_PER_PLAYER,
    ALLOCATION_DAILY_CAP,
    MATCH_XP_DAILY_CAP,
    DRILL_PER_PLAYER_DAILY_CAP,
    ApplyXpResult,
    cumulative_xp_for_level,
    drill_xp_reward,
    fusion_xp_reward,
    level_from_xp,
    match_xp_reward,
    scale_retroactive_reward,
    simulate_apply_card_xp,
    skill_points_earned_for_level,
    xp_needed_for_level,
    xp_progress,
)
from .mentor_math import (
    MENTOR_TRANSFERS_DAILY_LIMIT,
    SP_PER_MENTOR_UNIT,
    XP_PER_MENTOR_UNIT,
    MentorTransferPreview,
    is_mentor_source,
    is_mentor_target,
    mentor_max_units,
    mentor_units_to_sp,
    mentor_units_to_xp,
    preview_mentor_transfer,
    sp_to_mentor_units,
    xp_headroom_to_max,
)
from .drill_catalog import (
    DRILL_TIERS,
    STAT_DRILLS as DRILL_CATALOG,
    VALID_DRILL_IDS,
    drill_coin_cost,
    drill_spec,
    drill_unlocked,
)
from .identity import (
    ABANDONED_DAYS,
    INACTIVE_DAYS,
    IdentityStatus,
    classify_status,
    days_since_activity,
)
from .card_state import (
    CardStateFlags,
    can_perform_action,
    can_perform_action_from_flags,
    derive_modifiers,
    derive_overlay,
    derive_primary_state,
    detect_exclusive_conflict,
    exclusive_busy_proofs,
    has_exclusive_conflict,
)
from .club_state import (
    can_perform_club_action,
    derive_club_kind,
    derive_overlays as derive_club_overlays,
    soft_lifecycle_from_activity,
    soft_lifecycle_from_storage,
)
from .match_integrity import RecoveryAction, classify_interrupted_run
from .fatigue import (
    FATIGUE_BASE_DRAIN,
    FATIGUE_BENCH_PER_MATCH,
    FATIGUE_HOSPITAL_PER_DAY,
    FATIGUE_PASSIVE_BASE,
    FATIGUE_PASSIVE_PER_DAY,
    FATIGUE_PASSIVE_TG_PER_LEVEL,
    FATIGUE_RECOVERY_SESSION,
    TIER_DRAIN_BASE,
    TIER_PASSIVE_BASE,
    apply_bench_rest,
    apply_passive_recovery,
    apply_recovery_session,
    apply_starter_drain,
    recovery_batch_energy,
    recovery_session_eligible,
    clamp_fatigue,
    count_heavily_fatigued,
    fatigue_bar,
    fatigue_indicator,
    fatigue_stat_multiplier,
    match_fatigue_drain,
    passive_recovery_amount,
    stance_from_tactics_modifier,
    tactic_modifier,
)
from .intensity import (
    DIVISION_INTENSITY_TIER,
    INTENSITY_LABELS,
    clamp_intensity_tier,
    intensity_label,
    intensity_tier_for_division,
    intensity_vibe,
)
from .drill_caps import (
    CARD_DAILY_DRILL_LIMIT,
    CLUB_DAILY_DRILL_LIMIT,
    effective_daily_drill_count,
)
from .bench_rest import BENCH_REST_LIMIT, pick_bench_rest_ids
from .injury_math import (
    BASE_RECOVERY_DAYS,
    INJURY_ELIGIBLE_FATIGUE_BELOW,
    MODERATE_BASE_DAYS,
    SEVERITY_MULTIPLIER,
    TIER_INJURY_BASE,
    TIER_NAMES,
    facility_bonus_pct,
    fair_hospital_final_eta,
    fair_overflow_remaining_days,
    hospital_bed_capacity as injury_hospital_beds,
    hospital_recovery_multiplier as injury_hospital_mult,
    injury_chance,
    new_total_recovery_days,
    recovery_days_for_intensity,
    recovery_days_for_tier,
    roll_injury_tier,
    select_post_match_injury,
    should_early_discharge,
    untreated_base_days,
)
from .evolution_tracks import (
    CANCEL_FEE_COINS,
    EVOLUTION_START_COIN_MULTIPLIER,
    EVOLUTION_START_COOLDOWN_HOURS,
    EVOLUTION_START_ENERGY,
    EVOLUTION_START_FLAT,
    EVOLUTION_START_OVR_MULT,
    EVOLUTION_TRACKS,
    MAX_ACTIVE_EVOLUTIONS,
    VALID_TRACK_IDS,
    evolution_hub_start_cost_line,
    evolution_start_cost,
    evolution_unlocked,
    format_cooldown_remaining,
    track_goal,
    track_min_player_level,
)

__all__ = [
    "GameConfig",
    "DEFAULT_LIFECYCLE",
    "LifecyclePhase",
    "age_from_dob",
    "apply_xp_age_multiplier",
    "can_renew_contract",
    "ARCHETYPES",
    "ArchetypeDef",
    "CreatedPlayerCard",
    "archetypes_for",
    "balance_true_ovr",
    "create_player_card",
    "dob_from_age",
    "format_lifecycle_display",
    "lifecycle_phase",
    "lifecycle_phase_emoji",
    "lifecycle_phase_label",
    "roll_archetype",
    "roll_creation_age",
    "generate_youth_intake_cards",
    "POINTS_PER_OVR",
    "READY_OVR_DEFAULT",
    "READY_OVR_BY_LEVEL",
    "AGE_OUT_DEFAULT",
    "AGE_WARN_DEFAULT",
    "AGING_DECAY_MAX_DEFAULT",
    "GrowthResult",
    "academy_daily_points",
    "apply_academy_tick",
    "apply_academy_aging_decay",
    "ready_ovr_for_level",
    "star_band",
    "is_promotion_ready",
    "should_age_out",
    "should_age_warn",
    "init_visible_range",
    "narrow_range",
    "star_band_from_interval",
    "next_assessment_level",
    "DEEP_MIN_RANGE",
    "generate_regen_from_retired",
    "regen_rarity_for_ovr",
    "xp_multiplier",
    "yearly_stat_decline",
    "RARITY_POT_CAPS",
    "GeneratedPlayer",
    "apply_dynamic_potential_boost",
    "clamp_potential",
    "effective_potential",
    "format_potential_display",
    "generate_potential",
    "potential_tier_label",
    "rarity_potential_cap",
    "validate_potential_integrity",
    "calculate_level",
    "roll_dynamic_potential",
    "calculate_contract_renewal_cost",
    "calculate_true_ovr",
    "apply_match_form",
    "STAT_KEYS",
    "can_allocate_skill_point",
    "can_gain_stat_progression",
    "can_start_evolution_track",
    "detect_stat_inflation",
    "evolution_reward_steps",
    "evolution_track_stat_col",
    "rebalance_stats_to_ovr",
    "simulate_legacy_stat_drill",
    "stats_from_card",
    "L_MAX",
    "POINTS_PER_LEVEL",
    "FUSION_DAILY_LIMIT",
    "RETRO_SCALE_PCT",
    "RETRO_MAX_PER_PLAYER",
    "ALLOCATION_DAILY_CAP",
    "MATCH_XP_DAILY_CAP",
    "DRILL_PER_PLAYER_DAILY_CAP",
    "ApplyXpResult",
    "cumulative_xp_for_level",
    "drill_xp_reward",
    "fusion_xp_reward",
    "level_from_xp",
    "match_xp_reward",
    "scale_retroactive_reward",
    "simulate_apply_card_xp",
    "skill_points_earned_for_level",
    "xp_needed_for_level",
    "xp_progress",
    "MENTOR_TRANSFERS_DAILY_LIMIT",
    "SP_PER_MENTOR_UNIT",
    "XP_PER_MENTOR_UNIT",
    "MentorTransferPreview",
    "is_mentor_source",
    "is_mentor_target",
    "mentor_max_units",
    "mentor_units_to_sp",
    "mentor_units_to_xp",
    "preview_mentor_transfer",
    "sp_to_mentor_units",
    "xp_headroom_to_max",
    "DRILL_TIERS",
    "DRILL_CATALOG",
    "VALID_DRILL_IDS",
    "drill_coin_cost",
    "drill_spec",
    "drill_unlocked",
    "generate_player",
    "generate_squad",
    "EVOLUTION_TRACKS",
    "VALID_TRACK_IDS",
    "CANCEL_FEE_COINS",
    "MAX_ACTIVE_EVOLUTIONS",
    "EVOLUTION_START_COOLDOWN_HOURS",
    "EVOLUTION_START_ENERGY",
    "EVOLUTION_START_FLAT",
    "EVOLUTION_START_OVR_MULT",
    "EVOLUTION_START_COIN_MULTIPLIER",
    "evolution_hub_start_cost_line",
    "evolution_start_cost",
    "evolution_unlocked",
    "format_cooldown_remaining",
    "track_goal",
    "track_min_player_level",
    "FATIGUE_BASE_DRAIN",
    "FATIGUE_BENCH_PER_MATCH",
    "FATIGUE_HOSPITAL_PER_DAY",
    "FATIGUE_PASSIVE_BASE",
    "FATIGUE_PASSIVE_PER_DAY",
    "FATIGUE_PASSIVE_TG_PER_LEVEL",
    "FATIGUE_RECOVERY_SESSION",
    "TIER_DRAIN_BASE",
    "TIER_PASSIVE_BASE",
    "apply_bench_rest",
    "apply_passive_recovery",
    "apply_recovery_session",
    "apply_starter_drain",
    "recovery_batch_energy",
    "recovery_session_eligible",
    "clamp_fatigue",
    "count_heavily_fatigued",
    "fatigue_bar",
    "fatigue_indicator",
    "fatigue_stat_multiplier",
    "match_fatigue_drain",
    "passive_recovery_amount",
    "stance_from_tactics_modifier",
    "tactic_modifier",
    "DIVISION_INTENSITY_TIER",
    "INTENSITY_LABELS",
    "clamp_intensity_tier",
    "intensity_label",
    "intensity_tier_for_division",
    "intensity_vibe",
    "BASE_RECOVERY_DAYS",
    "MODERATE_BASE_DAYS",
    "SEVERITY_MULTIPLIER",
    "TIER_INJURY_BASE",
    "CARD_DAILY_DRILL_LIMIT",
    "CLUB_DAILY_DRILL_LIMIT",
    "BENCH_REST_LIMIT",
    "INJURY_ELIGIBLE_FATIGUE_BELOW",
    "TIER_NAMES",
    "facility_bonus_pct",
    "recovery_days_for_intensity",
    "untreated_base_days",
    "effective_daily_drill_count",
    "pick_bench_rest_ids",
    "fair_hospital_final_eta",
    "fair_overflow_remaining_days",
    "injury_chance",
    "new_total_recovery_days",
    "recovery_days_for_tier",
    "roll_injury_tier",
    "select_post_match_injury",
    "should_early_discharge",
    "INACTIVE_DAYS",
    "ABANDONED_DAYS",
    "IdentityStatus",
    "classify_status",
    "days_since_activity",
    "CardStateFlags",
    "derive_primary_state",
    "derive_overlay",
    "derive_modifiers",
    "can_perform_action",
    "can_perform_action_from_flags",
    "detect_exclusive_conflict",
    "exclusive_busy_proofs",
    "has_exclusive_conflict",
    "can_perform_club_action",
    "derive_club_kind",
    "derive_club_overlays",
    "soft_lifecycle_from_activity",
    "soft_lifecycle_from_storage",
    "RecoveryAction",
    "classify_interrupted_run",
]
